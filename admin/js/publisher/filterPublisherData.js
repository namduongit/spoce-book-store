...
export async function filterPublisher(){
    ...
    let show = document.querySelector("#show-slt-publisher").value.toLowerCase().trim();

    let publisherId = id !== '' ? id : '';
    let orderBy = 'maNXB', orderType = 'ASC';

    switch (sort.toLowerCase()) {
        case 'id giảm dần': orderType = 'DESC'; break;
        case 'tên nhà xuất bản tăng dần': orderBy = 'tenNXB'; break;
        case 'tên nhà xuất bản giảm dần': orderBy = 'tenNXB'; orderType = 'DESC'; break;
    ...
    if (publisherId) params.append("id_or_Name", publisherId);
    ...
    let url = `api/publishers/getPublisher.php?${params.toString()}`;
    ...
    try {
        ...
        await paginationPublisher(data.pageCount);
        return data.publisherList;
...
export function filterPublisherData() {
    const filterButton = document.querySelector("#filter-button-publisher");
...
async function paginationPublisher(pageCount) {
    if(pageCount > 1){

        let pagination_container = document.querySelector("#main__pagination_publisher");
