...
function returnJSONPublisher($filters, $pageCount) {
    ...
    echo json_encode(
        ["publisherList" => $response, "pageCount" => $pageCount],
...
$columns = [
    ...
    'nhaXuatBan.trangThai AS trangThai',
    'nhaXuatBan.ngayCapNhat AS ngayCapNhat' 
];
...
if (!empty($id_or_Name)) {
    $conditions[] = "(nhaXuatBan.tenNXB LIKE :name or nhaXuatBan.maNXB like :name)";
...
// print_r($result);
if (empty($result)) {
    echo json_encode(["publisherList" => [], "error" => "Không có NXB nào phù hợp!", "pageCount" => 0]);
    exit();
}
