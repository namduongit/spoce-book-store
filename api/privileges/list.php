...
function returnJSONPublisher($filters, $pageCount) {
    if (!is_array($filters) || empty($filters)) {
        ...
        echo json_encode(["error" => "Không tìm nhà thấy quyền!"]);
        exit();
    }
...
$orderByColumn = isset($_GET['orderByColumn']) ? trim($_GET['orderByColumn']) : 'maQuyen';
$orderType = isset($_GET['orderType']) ? trim($_GET['orderType']) : 'ASC';
$status = isset($_GET['status']) ? trim($_GET['status']) : '';
if (!empty($find)) {
    $conditions[] = "(quyen.maQuyen = :id or quyen.tenQuyen LIKE :name)";
    $params[':id'] = $find;  
    $params[':name'] = "%$find%";
}    

if (!empty($status)) {
    $conditions[] = "quyen.trangThai = :status";
...
if (empty($result)) {
    echo json_encode(["data" => [], "error" => "Không có quyền nào phù hợp!", "pageCount" => 0]);
    exit();
}
