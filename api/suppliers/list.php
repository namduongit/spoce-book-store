...
$conditions = [];
$params = [];
$limit = isset($_GET['limit']) ? (int)trim($_GET['limit']) : PHP_INT_MAX;
...
if (!empty($find)) {
    $conditions[] = "(nhaCungCap.maNCC = :id OR nhaCungCap.tenNCC LIKE :name)";
    $params[':id'] = $find;  
    $params[':name'] = "%$find%";  
}
if (!empty($status)) {
...
if (empty($result)) {
    echo json_encode(["data" => [], "error" => "Không có nhà cung cấp nào phù hợp!", "pageCount" => 0]);
    exit();
}
