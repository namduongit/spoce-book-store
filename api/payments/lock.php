...
if (empty($id)) {
    echo json_encode(["success" => false, "message" => "Thiếu ID phương thức."]);
    exit;
}

// Chuyển trạng thái phương thức
$status = ($status === 'Hoạt động') ? 'Tạm dừng' : 'Hoạt động';

try {
    // Khởi tạo model phương thức
    $model = new app_models_PhuongThucThanhToan();

    // Cập nhật trạng thái phương thức trong database
    $result = $model->updatePayment(
        $id,
        [
            "trangThai" => $status,
