...
if (empty($name) || empty($icon) || empty($status) || empty($option)) {
    echo json_encode([
        'success' => false,
        'message' => 'Thiếu thông tin phương thức',
        'title' => 'Cảnh báo',
        'type' => 'warning'
    ]);
    exit();
...
$insert = $payment_model->insertPayment($data);


if ($insert) {
    echo json_encode([
        'success' => true,
        'message' => 'Tạo phương thức mới thành công',
        'title' => 'Thông báo',
        'type' => 'success'
    ]);
    exit();
